...
namespace BinaryTrees
  ...
  data BTree : (leafType : Type) -> (nodeType : Type) -> Type where
      ...
      Node : nodeType -> -- node value
        BTree leafType nodeType -> -- left subtree
        BTree leafType nodeType -> -- right subtree
        BTree leafType nodeType

  %runElab derive "BTree" [Eq, Show]
  %name BTree bt, bt', bt''
  ...
  namespace LeavesOnly
    ...
    liftA2BTreeLeaf (Node n y z) (Node m v s)
      = Node n (liftA2BTreeLeaf y v) (liftA2BTreeLeaf z s) -- there's a choice here on what node to use! Maybe if we had a dot there?
