...
export prefix 0 #
record Prop (lprop : Vect m ((Type -> Type) -> Type)) where
  constructor (#)
  F : Type -> Type
  {auto 0 prf : All (\p => p F) lprop}

-- Maybe this ought to be a heterogeneous vector - if it's empty, no props?
||| Generalised tensors
||| For storing not necessarily cube-like structures
public export
data GenTensor : {lprop : Vect m ((Type -> Type) -> Type)} -> (shape : Vect n (Prop lprop)) -> (dtype : Type) -> Type where
    GTZ : (a : dtype) -> GenTensor [] dtype
    GTS : Functor f => f (GenTensor ds dtype) -> GenTensor ((# f) :: ds) dtype

%name GenTensor t, u, v

public export
data AllEq : (shape : Vect n (Prop [Functor])) -> (dtype : Type) -> Type where
  NilEq : Eq a => AllEq [] a
  ConsEq : Functor f => Eq (f (GenTensor fs dtype)) => AllEq ((# f) :: fs) dtype
...
public export
data AllShow : (shape : Vect n (Prop [Functor])) -> (dtype : Type) -> Type where
  NilShow : Show a => AllShow [] a
  ConsShow : Functor f => Show (f (GenTensor fs dtype)) => AllShow ((# f) :: fs) dtype
...
-- foldrGenFoldable : {shape : Vect n (Prop [Functor, Foldable])} -> (f : a -> b -> b) -> (z : b) -> GenTensor shape a -> b
...
-- public export
-- data AllFoldable : (shape : Vect n (Prop Functor)) -> (dtype : Type) -> Type where
--     NilFoldable : AllFoldable [] a
--     ConsFoldable : (Foldable f, Functor f) => f (AllFoldable ds dtype) -> AllFoldable ((# f) :: ds) dtype
-- 
-- 
